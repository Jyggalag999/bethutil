...
static void set_gamma_correction(mipmapped_texture::resample_params &params, const TextureType tex_type)
{
...
auto generate_mipmaps(CrunchTexture &&file) -> ResultCrunch
{
    ...
    gen_params.m_max_mips      = cCRNMaxLevels;
    gen_params.m_min_mip_size  = 1;

    set_gamma_correction(gen_params, file.get_texture_type());

    if (const auto success = file.get().generate_mipmaps(gen_params, true); !success)
    {
        // No error information is propagated from Crunch.
        return tl::make_unexpected(error_from_hresult(E_UNEXPECTED));
    }
...
auto convert(CrunchTexture &&file, const DXGI_FORMAT format) -> ResultCrunch
{
    ...
    pack_params.m_num_helper_threads = (threads != 0 ? threads : 1) - 1;
